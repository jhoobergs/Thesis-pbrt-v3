...
namespace pbrt {
    ...
    BSPNodeBasedWithKd::BSPNodeBasedWithKd(std::vector<std::shared_ptr<pbrt::Primitive>> p, uint32_t isectCost,
    ...
    void BSPNodeBasedWithKd::buildTree() {
        ...
        while (!stack.empty()) { // || nodeNum > 235000 || nodeNum > 1400000
            ...
            nodeDirections.emplace_back(1,0,0);
            nodeDirections.emplace_back(0,1,0);
            nodeDirections.emplace_back(0,0,1);
            if(Kmeans > 0) {
                auto generatedNodeDirections = calculateDirections(gen, Kmeans, primitives, currentBuildNode.primNums,
